...
def run_evaluation(
    ...
    corpus_embeddings = embedder.embed([item_text(i) for i in corpus])
    ...
    for q in queries:
        ...
        results.append(QueryResult(q["query"], q["expected_item_id"], ranked))
    return results
...
def main() -> None:
    ...
    for name, embedder in models:
        results = run_evaluation(embedder, corpus, queries)
