...
def recall_at_k(results: List[QueryResult], k: int) -> float:
    if not results:
        return 0.0
    hits = sum(1 for r in results if r.expected_item_id in r.ranked_item_ids[:k])
...
def mrr_at_k(results: List[QueryResult], k: int) -> float:
    ...
    rr_sum = 0.0
    for r in results:
        top_k = r.ranked_item_ids[:k]
        if r.expected_item_id in top_k:
            rr_sum += 1.0 / (top_k.index(r.expected_item_id) + 1)
